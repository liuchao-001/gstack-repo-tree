...
This tree describes the [garrytan/gstack](https://github.com/garrytan/gstack) skill
suite bound as a `standalone-source` repo under `/Users/super/Workspace/gstack-repo`.

- Treat the bound source repo as read-mostly upstream; record decisions here, not in the source repo's framework files.
- Baseline tree coverage is intentionally scaffold-only. Grow domains by concern (what an agent needs to know to decide) — e.g. `skills/`, `cli/`, `docs/` — not by mirroring the source repo's directory layout.
- The single active member is `liuchao-001` (tree maintainer); add new members under `members/<login>/NODE.md` before assigning ownership on new domains.
